...
void DrawWidget::paintEvent(QPaintEvent *event){
    ...
    if(pTask){
        ...
        for (int i=0;i<pTask->result_vector.size();i++){
            int i_ = pTask->result_vector[i].i;
            int j_ = pTask->result_vector[i].j;

            p.drawLine(pTask->vector_of_points[i_].x, pTask->vector_of_points[i_].y,
                               pTask->vector_of_points[j_].x, pTask->vector_of_points[j_].y);
